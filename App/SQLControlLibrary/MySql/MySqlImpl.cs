...
namespace SQLControlLibrary.SQL
{
    ...
    public class MySqlImpl : ISql
    {
        ...
        private MySqlDataReader m_dataReader = null;
        ...
        public EnumReturnVal Init()
        {
            ...
            {
                constrc = CONNECTSQL;
                CONNECTSQLSTR = $"server={CONNECTSQL.host};port={CONNECTSQL.port};user={CONNECTSQL.username};password={CONNECTSQL.password};Allow User Variables=True;SslMode=None";
                m_connection = new MySqlConnection(CONNECTSQLSTR);
                return EnumReturnVal.ERROR_OK;
            }
            catch (Exception ex)
            {
                SMLogWindow.OutLog($"{constrc.database}:{constrc.tablename}:{ex.ToString()}", Color.Red);
        ...
        public EnumReturnVal CreateDataBase(string databaseName)
        {
            try
            {
                ...
                sqlstr = $"create database if not exists {databaseName}";

                SMLogWindow.OutLog($"database:{constrc.database}:tablename:{constrc.tablename}:sql:{sqlstr}", Color.Red);
        ...
        public EnumReturnVal InsertData<T>(string sqlSheet, Dictionary<string, T> dicData)
        {
            try
            {
                ...
                SMLogWindow.OutLog($"database:{constrc.database}:tablename:{constrc.tablename}:{sqlstr}", Color.Green);
                return Excute(sqlstr);
            }
            catch (Exception ex)
            {
        ...
        public EnumReturnVal DeleteData(string sqlSheet, Dictionary<string, SqlCmdInfo> dicdata = null)
        {
            try
            {
                ...
                SMLogWindow.OutLog($"database:{constrc.database}:tablename:{constrc.tablename}:sql:{sqlstr}",Color.Green);
                return Excute(sqlstr);
        ...
        public EnumReturnVal ReadData(string sqlSheet, string[] arr)
        {
            ...
            if (sqlSheet != null)
            {
                try
                {
                    ...
                    string sqlstr = GetReadsqlContent(sqlSheet, arr);

                    SMLogWindow.OutLog($"database:{constrc.database}:tablename:{constrc.tablename}:{sqlstr}", Color.Green);
                    return Excute(sqlstr, true);
        ...
        private EnumReturnVal CreateTable(string tableName, string[] arr)
        {
            try
            {
                ...
                string sqlstr = $"create table if not exists {tableName}(`ID` INT UNSIGNED AUTO_INCREMENT,";
                //sqlstr += "`product_datetime` DATETIME not null,";
                ...
                sqlstr += $"PRIMARY KEY( `ID`))ENGINE = InnoDB DEFAULT CHARSET = utf8";
        ...
        private string GetInsertContent<T>(string sqlSheet, Dictionary<string, T> dicData)
        {
           ...
           // sqlstr += $")values('{DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")}','";
        ...
        private EnumReturnVal Excute(string sqlText, bool bReadSql = false)
        {
            try
            {
                lock (obj)
                {
                    ...
                    if (m_connection == null) m_connection = new MySqlConnection(CONNECTSQLSTR);
                    ...
                    if (bReadSql)
                    {
                        MySqlDataAdapter adap = new MySqlDataAdapter(m_command);                       
