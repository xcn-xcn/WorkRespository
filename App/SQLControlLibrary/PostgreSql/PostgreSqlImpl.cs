...
namespace SQLControlLibrary.SQL
{
    ...
    public class PostgreSqlImpl : ISql
    {
        ...
        private NpgsqlCommand m_command = null;
        private NpgsqlDataReader m_dataReader = null;
        ...
        public EnumReturnVal Init()
        {
            ...
            {
                constrc = CONNECTSQL;
                CONNECTSQLSTR = $"server={CONNECTSQL.host};port={CONNECTSQL.port};user id={CONNECTSQL.username};password={CONNECTSQL.password}";
                m_connection = new NpgsqlConnection(CONNECTSQLSTR);
        ...
        private EnumReturnVal CreateTable(string tableName, string[] arr)
        {
            try
            {
                ...
                string sqlstr = $"create table if not exists {tableName}(ID BIGSERIAL not null,";
        ...
        private EnumReturnVal Excute(string sqlText, bool bReadSql = false)
        {
            try
            {
                lock (obj)
                {
                    ...
                    if (m_connection == null) m_connection = new NpgsqlConnection(CONNECTSQLSTR);
                    ...
                    if (bReadSql)
                    {
                        NpgsqlDataAdapter adap = new NpgsqlDataAdapter(m_command);                       
